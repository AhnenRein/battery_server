...
# 初始化数据
    ...
    return time_array, last_alt, last_neu, kap_neu, soc,last_bat[c_t],except_bat[c_t],full_bat[c_t]
    ...
    for alt, c_t in zip(last_alt, time_array[1:]):
        if alt > 0: # 电网供电给用电器,考虑电池的放电，同模式A
            ...
            if kap_alt == 0: # 当前电池电量为零的情况，取旧值
                ...
                last_neu.append(alt - ew)
                kap_neu.append(kap_alt - ew / 4)
                soc.append(((kap_neu(-1) - ew) / max_kap) * 100)
...
def modelF(table, cur_model):
    ...
    for alt, c_t，preis in zip(last_alt, time_array,preis_arry):
        c_time = datetime.strptime(c_t,'%H:%M:%S').time()#提取选中的last_alt对应的时间。   
        a_preis=preis
...
#限制
    ...
    if last_alt > 0:# last_alt大于0的情况，电网供电给用电器,考虑电池的放电
        ...
        if abs(alt) > AB and max_kap!==kap_alt:#超过界限，电池没充满，给电池充电
            print("太阳能2电池")
            if dif_last < l_last:
